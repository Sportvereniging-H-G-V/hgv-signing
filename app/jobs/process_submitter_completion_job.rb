...
class ProcessSubmitterCompletionJob
  ...
  def perform(params = {})
    ...
    if is_all_completed && submitter.completed_at == submitter.submission.submitters.maximum(:completed_at)
      # Combine PDF result is always enabled by default
      config = submitter.submission.account.account_configs.find_by(key: AccountConfig::COMBINE_PDF_RESULT_KEY)
      Submissions::EnsureCombinedGenerated.call(submitter) if config.nil? || config.value != false
