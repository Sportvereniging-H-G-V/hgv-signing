        ...
        <form @submit.prevent="validateSaveAndClose">
          <div class="my-4">
            ...
            >
              ...
              <input
                v-if="conditionField(condition)?.type === 'date' && ['age_less_than', 'age_greater_than'].includes(condition.action)"
                v-model.number="condition.value"
                type="number"
                min="0"
                max="150"
                class="input input-bordered input-sm w-full bg-white h-11 pl-4 text-base font-normal"
                :placeholder="t('age_in_years')"
                required
              >
            </div>
          </div>
  ...
  methods: {
    ...
    fieldActions (field) {
      ...
      if (field.type === 'checkbox') {
        ...
        actions.push('not_empty', 'empty', 'age_less_than', 'age_greater_than')
      } else {
