<template>
  ...
  >
    ...
    >
      <button
        v-if="fieldTypes.includes(type) || ((withPayment || type != 'payment') && (withVerification || type != 'verification'))"
        ...
        @dragend="$emit('drag-end')"
        @click="['payment', 'verification'].includes(type) ? $emit('add-field', type) : $emit('set-draw-type', type)"
      >
        <div
