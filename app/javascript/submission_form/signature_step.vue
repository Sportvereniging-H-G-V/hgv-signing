<template>
  ...
  >
    ...
    >
      <div
        v-if="!modelValue && !computedPreviousValue && !isTextSignature && isSignatureStarted"
...
export default {
  ...
  methods: {
    ...
    maybeSetSignedUuid (signedUuid) {
      try {
        // Signature remembering functionality removed
    ...
    async submit () {
      ...
      return new Promise((resolve, reject) => {
        cropCanvasAndExportToPNG(this.$refs.canvas, { errorOnTooSmall: false }).then(async (blob) => {
          ...
          if (this.dryRun) {
            ...
            // formData.append('remember_signature', this.rememberSignature) // Removed
            ...
            return fetch(this.baseUrl + '/api/attachments', {
              ...
              if (resp.status === 422 || resp.status === 500) {
                if (isJson) {
                  ...
                  return Promise.reject(new Error(data.error || 'Upload failed'))
                } else {
              ...
              if (!resp.ok) {
                return Promise.reject(new Error(`Upload failed with status ${resp.status}`))
              }
          ...
          if (this.field.required === true) {
            // Alleen bij required velden een error tonen
            if (error.message === 'Canvas is empty') {
            ...
            // Bij optionele velden gewoon doorlaten
            return resolve({})
          }
        })
      })
