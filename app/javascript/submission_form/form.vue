  ...
  >
    ...
    >
      ...
      <InviteForm
        ...
        :optional-submitters="filteredOptionalInviteSubmitters"
        :submitter-slug="submitterSlug"
        :authenticity-token="authenticityToken"
        :url="baseUrl + submitPath + '/invite'"
        :all-fields="allFieldsComputed"
        :all-submitters-values="allSubmittersValuesComputed"
        :style="{ maxWidth: isBreakpointMd ? '582px' : '' }"
      ...
      <FormCompleted
        ...
        :has-signature-fields="fields.some((f) => ['signature', 'initials'].includes(f.type))"
...
export default {
  ...
  props: {
    ...
    // rememberSignature removed - functionality disabled
    minimize: {
      type: Boolean,
      required: false,
      default: false
  ...
  computed: {
    ...
    emptyValueRequiredStep () {
      return this.stepFields.find((fields, index) => {
        return fields.some((f) => {
          // Only check required fields that are actually visible (conditions met)
          // Note: stepFields already filters on checkFieldConditions, but we check again
          // to ensure the field is still visible with current values
          const isVisible = this.checkFieldConditions(f) && this.checkFieldDocumentsConditions(f)
          return f.required && isVisible && isEmpty(this.values[f.uuid])
        })
      })
    },
    submitButtonText () {
    ...
    allSubmittersValuesComputed () {
      // Merge server-provided allSubmittersValues (static, contains all submitters' values at page load)
      // with current reactive this.values (updates as user fills the form)
      // This ensures conditions re-evaluate when the current submitter changes field values
      if (this.allSubmittersValues && Object.keys(this.allSubmittersValues).length > 0) {
        // Start with server-provided values (includes other submitters' values)
        const merged = { ...this.allSubmittersValues }
        
        // Override with current submitter's reactive values (updates as user types)
        // Only update values for fields that belong to the current submitter
        Object.keys(this.values).forEach((fieldUuid) => {
          // Only merge values for fields that belong to the current submitter
          // This prevents accidentally overwriting other submitters' values
          if (this.fieldsUuidIndex[fieldUuid]) {
            merged[fieldUuid] = this.values[fieldUuid]
          }
        })
      ...
      // Fallback: if server doesn't provide allSubmittersValues, only use current submitter's values
      // Note: This fallback is limited because readonlyFields only contains fields from the current submitter,
      // so we cannot reliably get values from other submitters without server-provided data
      return { ...this.values }
    },
    allFieldsComputed () {
      // Use the server-provided allFields if available, otherwise fall back to current submitter's fields
      return this.allFields && this.allFields.length > 0 ? this.allFields : this.fields
    },
    filteredOptionalInviteSubmitters () {
      if (!this.optionalInviteSubmitters || !this.optionalInviteSubmitters.length) {
        return []
      }

      return this.optionalInviteSubmitters.filter((submitter) => {
        // If submitter has conditions, check them
        if (submitter.conditions && submitter.conditions.length) {
          const result = submitter.conditions.reduce((acc, condition) => {
            if (condition.operation === 'or') {
              acc.push(acc.pop() || this.checkFieldConditionWithAllValues(condition))
            } else {
              acc.push(this.checkFieldConditionWithAllValues(condition))
            }
            return acc
          }, [])
        ...
        // Use allFields to include fields from all submitters for condition evaluation
        const submitterFields = this.allFieldsComputed.filter((f) => f.submitter_uuid === submitter.uuid)
        if (submitterFields.some((f) => f.conditions?.length)) {
          // At least one field should be visible (either no condition or condition met)
          return submitterFields.some((field) => {
            if (field.conditions?.length) {
              return this.checkFieldConditionsWithAllValues(field)
            } else {
  ...
  methods: {
    ...
    checkFieldConditions (field) {
      return this.checkFieldConditionsWithValues(field, this.values)
    },
    checkFieldConditionsWithAllValues (field) {
      return this.checkFieldConditionsWithValues(field, this.allSubmittersValuesComputed)
    },
    checkFieldConditionsWithValues (field, valuesToUse) {
      if (field.conditions?.length) {
        // Backend logic: conditions are evaluated sequentially
        // When an 'or' operation is encountered, it combines with the LAST result (acc.pop || result)
        // Then .exclude?(false) means all results must be true
        const results = []
        
        field.conditions.forEach((cond) => {
          const result = this.checkFieldConditionWithValues(cond, valuesToUse)
          
          if (cond.operation === 'or') {
            // Combine with the last result (OR operation)
            results.push(results.pop() || result)
          } else {
            // Add to results (AND operation - all must be true)
            results.push(result)
          }
        })
        
        // All results must be true (no false values)
    ...
    checkFieldCondition (condition) {
      return this.checkFieldConditionWithValues(condition, this.values)
    },
    checkFieldConditionWithAllValues (condition) {
      return this.checkFieldConditionWithValues(condition, this.allSubmittersValuesComputed)
    },
    checkFieldConditionWithValues (condition, valuesToUse) {
      // Only use fieldsUuidIndex (which contains only current submitter's fields) for security
      // For fields from other submitters, use allFieldsComputed to get the field definition
      const field = this.fieldsUuidIndex[condition.field_uuid] || 
                    (this.allFieldsComputed.find((f) => f.uuid === condition.field_uuid))

      // Get the field value first to check if it exists
      const defaultValue = !field || isEmpty(field.default_value) ? null : field.default_value
      const fieldValue = valuesToUse[condition.field_uuid] ?? defaultValue

      // If condition references a field from the current submitter that hasn't been filled yet,
      // we need to evaluate the condition based on the actual value (which may be null/undefined).
      // For 'not_empty' conditions on untouched fields, return false (field is empty, condition not met).
      // This ensures that conditional fields only become visible when their conditions are actually met.
      // SECURITY: Only allow this if the field is in fieldsUuidIndex (current submitter's fields)
      if (field && this.fieldsUuidIndex[condition.field_uuid] && field.submitter_uuid === this.submitter.uuid) {
        // Check if the field has been explicitly set in this.values (user interaction)
        // Don't consider defaultValue as "touched" - only user-set values count
        const isTouched = condition.field_uuid in this.values
        // If the field hasn't been touched, evaluate based on the actual value (null/undefined = empty)
        // For 'not_empty' conditions, an untouched field is considered empty (false)
        if (!isTouched) {
          // For 'not_empty' conditions, untouched fields are empty, so condition is false
          if (condition.action === 'not_empty') {
            return false
          }
          // For 'empty' conditions, untouched fields are empty, so condition is true
          if (condition.action === 'empty') {
            return true
          }
          // For other conditions, continue with normal evaluation below
        }
        // Otherwise, continue with normal evaluation below
      }

      if (['not_empty', 'checked', 'equal', 'contains'].includes(condition.action) && field && !this.checkFieldConditionsWithValues(field, valuesToUse)) {
        return false
      }
      ...
      if (['empty', 'unchecked'].includes(condition.action)) {
        ...
        // For 'not_empty' conditions, check if fieldValue is not empty
        // Empty string '' is considered empty, so condition is false
        return !isEmpty(fieldValue)
      } else if (['equal', 'contains'].includes(condition.action) && field) {
        if (field.options) {
          const option = field.options.find((o) => o.uuid === condition.value)
          const values = [fieldValue].flat()

          return values.includes(this.optionValue(option, field.options.indexOf(option)))
        } else {
          return [fieldValue].flat().includes(condition.value)
        }
        ...
        return !values.includes(this.optionValue(option, field.options.indexOf(option)))
      } else if (['age_less_than', 'age_greater_than'].includes(condition.action) && field && field.type === 'date' && fieldValue) {
        const age = this.calculateAge(fieldValue)
        const thresholdAge = parseInt(condition.value, 10)

        if (isNaN(age) || isNaN(thresholdAge)) {
          return false
        }

        if (condition.action === 'age_less_than') {
          return age < thresholdAge
        } else if (condition.action === 'age_greater_than') {
          return age > thresholdAge
        }
      }
    ...
    async submitStep (e) {
      ...
      stepPromise().then(async () => {
        ...
        // Voeg alleen expliciet values toe voor velden die geen eigen form inputs hebben
        // (zoals signature / initials). Voor andere typen (zoals multiple) vertrouwt
        // de backend op de bestaande inputs (bijv. values[uuid][]).
        this.currentStepFields.forEach((field) => {
          if (!['signature', 'initials'].includes(field.type)) return

          const value = this.values[field.uuid]

          if (value !== undefined && value !== null && value !== '') {
            if (!formData.has(`values[${field.uuid}]`)) {
              formData.append(`values[${field.uuid}]`, value)
            }
          }
        })

        if (isLastStep && !emptyRequiredField && !this.inviteSubmitters.length && !this.filteredOptionalInviteSubmitters.length) {
          formData.append('completed', 'true')
          formData.append('timezone', Intl.DateTimeFormat().resolvedOptions().timeZone)
        }

        const saveStepRequest = this.saveStep(formData)
        ...
        await saveStepRequest.then(async (response) => {
          ...
          } else if (this.inviteSubmitters.length || this.filteredOptionalInviteSubmitters.length) {
            this.isInvite = true
          } else {
