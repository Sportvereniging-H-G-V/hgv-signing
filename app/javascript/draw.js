...
window.customElements.define('draw-signature', class extends HTMLElement {
  ...
  async submitImage () {
    // Validatie verwijderd - alleen controleren of er iets is getekend
    if (this.pad.isEmpty()) {
      alert('Please draw your signature first.')

      return Promise.reject(new Error('Signature is empty'))
    }
    ...
    return cropCanvasAndExportToPNG(this.canvas).then(async (blob) => {
      ...
      // formData.append('remember_signature', 'true') // Removed
