...
class SubmittersController < ApplicationController
  ...
  def update
    ...
    return handle_update_error(submission, :submitter_cannot_be_updated) if cannot_update_submitter?

    return handle_update_error(submission, :at_least_one_field_must_be_filled) if submitter_params.values.all?(&:blank?)

    handle_editor_message_restrictions
    update_submitter_preferences
  ...
  def handle_update_error(submission, error_key)
    redirect_back fallback_location: submission_path(submission), alert: I18n.t(error_key)
  end

  def handle_editor_message_restrictions
    if current_user.role == User::EDITOR_ROLE
      params.delete(:subject)
      params.delete(:body)
      params.delete(:is_custom_message)
    elsif params[:is_custom_message] != '1'
      params.delete(:subject)
      params.delete(:body)
    end
  end

  def update_submitter_preferences
