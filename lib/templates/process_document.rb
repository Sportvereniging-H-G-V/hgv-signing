...
module Templates
  module ProcessDocument
    ...
    def generate_document_preview_images(attachment, data, range, concurrency: CONCURRENCY)
      # Skip PDF processing in test environment if Pdfium is not available
      if (Rails.env.test? && !defined?(Pdfium)) || (defined?(Pdfium) && !Pdfium.instance_variable_get(:@pdfium_loaded))
        Rails.logger.warn('Skipping PDF preview generation - Pdfium not available') if defined?(Rails.logger)
        return
      end
