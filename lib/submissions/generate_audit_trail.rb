  ...
  module GenerateAuditTrail
    ...
    def call(submission)
      ...
      I18n.with_locale(last_submitter.metadata.fetch('lang', account.locale)) do
        ...
        if pkcs
          sign_params =
            Submissions::GenerateResultAttachments
            .build_signing_params(last_submitter, pkcs, tsa_url)
            .merge(
              reason: '' # Lege string om te voorkomen dat HexaPDF een default reason gebruikt
            )
        ...
        ActiveStorage::Attachment.create!(
          blob: ActiveStorage::Blob.create_and_upload!(
            io: io, filename: "#{I18n.t('audit_log')} - " \
        ...
        submission.template_fields.filter_map do |field|
          ...
          [
            ...
            if field['type'].in?(%w[image signature initials stamp]) &&
              ...
              # Toon metadata alleen voor signature velden in audit trail
              if field['type'] == 'signature' && (with_signature_id || field.dig('preferences', 'reason_field_uuid'))
                reason_value = submitter.values[field.dig('preferences', 'reason_field_uuid')].presence
                timezone = with_submitter_timezone ? (submitter.timezone || account.timezone) : account.timezone

                metadata_text = []

                metadata_text << "ID: #{attachment.uuid}" if with_signature_id

                if with_signature_id_reason
                  reason_string = if reason_value
                                    "#{I18n.t('reason')}: #{reason_value}"
                                  else
                                    "#{I18n.t('digitally_signed_by')} #{submitter.name}"
                                  end
                  reason_string += " <#{submitter.email}>" if submitter.email.present?
                  metadata_text << reason_string
                end

                formatted_time = I18n.l(attachment.created_at.in_time_zone(timezone),
                                        format: :long, locale: account.locale)
                timezone_abbr = TimeUtils.timezone_abbr(timezone, attachment.created_at)
                metadata_text << "#{formatted_time} #{timezone_abbr}"

                composer.formatted_text_box(
                  [{ text: metadata_text.join("\n"), font_size: 7 }],
                  padding: [0, 0, 10, 0]
                )
              else
    ...
    def add_logo(column, _submission = nil)
      ...
      column.formatted_text([{ text: 'HGV Signing',
