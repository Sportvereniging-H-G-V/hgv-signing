...
module Submissions
  module GenerateResultAttachments
    ...
    SIGN_REASON = 'Signed by %<name>s with HGV Signing.com'
      ...
      (submission.template_fields || submission.template.fields).each do |field|
        ...
        field.fetch('areas', []).each do |area|
          ...
          when ->(type) { type == 'signature' && (with_signature_id || field.dig('preferences', 'reason_field_uuid')) }
            # Metadata wordt alleen in audit trail getoond, niet op het document zelf
            # Alleen de handtekening afbeelding wordt op het document getoond
            ...
            # Alleen de handtekening afbeelding tekenen, zonder metadata tekst
            ...
            canvas.image(
              ...
              at: [
                ...
                height - (area['y'] * height) - (area['h'] * height / 2) - ((image.height * scale) / 2)
              ],
    ...
    def build_pdf_attachment(pdf:, submitter:, pkcs:, tsa_url:, uuid:, name:)
      ...
      # PDF wordt nog steeds ondertekend, maar zonder reason tekst in metadata
      if pkcs
        sign_params = build_signing_params(submitter, pkcs, tsa_url).merge(
      ...
      ActiveStorage::Attachment.new(
        blob: ActiveStorage::Blob.create_and_upload!(io: io, filename: "#{name}.pdf"),
