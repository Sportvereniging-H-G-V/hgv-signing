...
module PhoneLengths
  ...
  DEFAULT_LENGTH = { min: 7, max: 15 }.freeze

  module_function

  def length_for_dial_code(dial_code)
    LENGTHS[dial_code] || DEFAULT_LENGTH
  end

  def valid_length?(phone_number, dial_code)
    return true if phone_number.blank?

    # Haal alleen cijfers uit het telefoonnummer
    digits = phone_number.to_s.gsub(/[^\d]/, '')
    return false if digits.blank?

    # Verwijder de landcode van het begin
    dial_code_str = dial_code.to_s
    local_number = if digits.start_with?(dial_code_str)
                     digits[dial_code_str.length..]
                   else
                     digits
                   end

    return false if local_number.blank?

    length = local_number.length
    rules = length_for_dial_code(dial_code_str)

    length.between?(rules[:min], rules[:max])
  end

  def extract_dial_code(phone_number)
    # Probeer de landcode te detecteren door te beginnen met de langste codes
    sorted_codes = LENGTHS.keys.sort_by(&:length).reverse

    sorted_codes.each do |code|
      phone_digits_plus = phone_number.to_s.gsub(/[^\d+]/, '')
      phone_digits = phone_number.to_s.gsub(/[^\d]/, '')
      return code if phone_digits_plus.start_with?("+#{code}") || phone_digits.start_with?(code)
    end
