...
module Accounts
  ...
  def create_duplicate(account)
    ...
    new_user.email = "#{SecureRandom.hex}@hgv-signing.local"
  ...
  def link_expires_at(account)
    # Download links expire is always enabled by default
    config = AccountConfig.find_by(account: account, key: AccountConfig::DOWNLOAD_LINKS_EXPIRE_KEY)
    return LINK_EXPIRES_AT.from_now if config.nil? || config.value != false
