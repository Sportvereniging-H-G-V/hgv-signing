...
class Pdfium
  ...
  begin
    ...
    # In test environment, allow graceful failure if pdfium is not available
    ...
    Rails.logger.warn("Pdfium library not available in test environment: #{e.message}") if defined?(Rails.logger)
    @pdfium_loaded = false
    # Stub all attach_function calls to create no-op methods
    def self.attach_function(name, *args)
      define_singleton_method(name) do |*_method_args|
        # Return appropriate default values based on return type
        return nil unless args.last.is_a?(Symbol)

        case args.last
        when :FPDF_DOCUMENT, :FPDF_PAGE, :FPDF_BITMAP, :FPDF_FORMHANDLE,
             :FPDF_TEXTPAGE, :FPDF_PAGEOBJECT, :FPDF_PATHSEGMENT, :pointer
          FFI::Pointer::NULL
        when :int, :ulong, :FPDF_BOOL
          0
        when :float, :double
          0.0
        end
