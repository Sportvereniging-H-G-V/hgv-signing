...
RSpec.describe 'Signing Form' do
  ...
  context 'when the template form link is opened' do
    ...
    it 'completes the form when identity verification with a 2FA code is enabled', sidekiq: :inline do
      ...
      expect(page).to have_css('.completed-form-message-title', text: 'Document has been signed!', wait: 10)
  ...
  context 'when the signature step' do
    ...
    it 'disables the button if the canvas is not drawn' do
      visit submit_form_path(slug: submitter.slug)

      find('#expand_form_button').click
      # Canvas is niet getekend (leeg)

      # De knop moet disabled zijn wanneer de canvas leeg is
      submit_button = find('#submit_form_button', visible: :all)
